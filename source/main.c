...
void fix_button_assign() {
    ...
    if (ret < 0) { printf("  ERROR: OpenRegistry (0x%08X)\n", ret); return; }
    ret = sceRegOpenCategory(h, "/CONFIG/SYSTEM/XMB", 1, &hd);
    if (ret < 0) { printf("  ERROR: OpenCategory (0x%08X)\n", ret); sceRegCloseRegistry(h); return; }
    ret = sceRegGetKeyInfo(hd, "button_assign", &hk, NULL, NULL);
    if (ret < 0) { printf("  ERROR: GetKeyInfo (0x%08X)\n", ret); sceRegCloseCategory(hd); sceRegCloseRegistry(h); return; }
    unsigned int current = 0;
    ret = sceRegGetKeyValue(hd, hk, &current, sizeof(current));
    if (ret < 0) { printf("  ERROR: GetKeyValue (0x%08X)\n", ret); sceRegCloseCategory(hd); sceRegCloseRegistry(h); return; }
    if (current == 0) {
        printf("  Already X=Confirm. OK\n");
    } else {
        unsigned int new_val = 0;
        ret = sceRegSetKeyValue(hd, "button_assign", &new_val, sizeof(new_val));
        printf("  Swapped to X=Confirm: %s\n", ret < 0 ? "ERROR" : "OK");
    }
...
void set_username() {
    ...
    if (ret < 0) { printf("  ERROR: OpenCategory (0x%08X)\n", ret); sceRegCloseRegistry(h); return; }
    ret = sceRegSetKeyValue(hd, "name", OWNER_NAME, sizeof(OWNER_NAME));
    printf("  Username: %s\n", ret < 0 ? "ERROR" : "OK");
    sceRegFlushCategory(hd); sceRegCloseCategory(hd); sceRegCloseRegistry(h);
}
